...
MainWindow::MainWindow(QWidget *parent)
  ...
  group_signals();
  ui->doubleSpinBox_xmax->setRange(-std::numeric_limits<double>::max(),
                                   std::numeric_limits<double>::max());
  ui->doubleSpinBox_xmin->setRange(-std::numeric_limits<double>::max(),
                                   std::numeric_limits<double>::max());
  ui->doubleSpinBox_ymax->setRange(-std::numeric_limits<double>::max(),
                                   std::numeric_limits<double>::max());
  ui->doubleSpinBox_ymin->setRange(-std::numeric_limits<double>::max(),
                                   std::numeric_limits<double>::max());
  ui->doubleSpinBox_xValue->setRange(-std::numeric_limits<double>::max(),
                                     std::numeric_limits<double>::max());
}
...
void MainWindow::group_signals() {
  ...
  connect(ui->pushButton_detete, SIGNAL(clicked()), this,
          SLOT(delete_symbol()));
...
void MainWindow::delete_symbol() {
  QString newText =
      ui->lineEdit->text().left(std::size(ui->lineEdit->text()) - 1);
...
void MainWindow::on_pushButton_calc_clicked() {
  ...
  if (term <= 60) {
    ...
    if (ui->radioButton_a->isChecked()) {
      ...
      ui->lineEdit_monthlyPayment->setText(
          QString::number(credit.mounthly_payment[0]) + "..." +
          QString::number(credit.mounthly_payment[term - 1]));
    }
    ui->lineEdit_overpayment->setText(
        QString::number(credit.overpayment_credit));
...
void MainWindow::on_pushButton_depositCalc_clicked() {
  ...
  ui->lineEdit_accruedInterest->setText(
      QString::number(depositOut.accruedInterest));
